...
Creates a list of random values between 0.01 and 0.05, suitable for visually exploding slices of a pie chart. The number of values in the list is determined by the `pie_size` argument.
...
An integer representing the number of slices in the pie chart.

Returns
-------

A list of floating-point values between 0.01 and 0.05, with a length equal to `pie_size`.
...
A dictionary where keys represent unique values from the column and values represent their normalized counts (fraction of total rows).
...
The numerical value to convert to a percentage.
...
The formatted percentage string (e.g., "42%").
...
A dictionary where keys represent unique values from the column and values represent their counts.
...
The input DataFrame, containing a 'Category' column and a 'count' column.

**`col`** : `str`

Unused in this function, but kept for consistency with other charting functions.
...
True to display a legend, False to hide it.
...
The label for the column containing unique values in the output DataFrame.

**`y_label`** : `str`

The label for the column containing counts in the output DataFrame.
...
A new DataFrame with two columns: - x\_label: Contains the unique values from the specified column. - y\_label: Contains the counts of those values.

Example
-------

> > > import pandas as pd df = pd.DataFrame({'educacion': \['A;B', 'A', 'C;B', 'D'\]}) new\_df = make\_df(df, "educacion", "categories", "count") print(new\_df) categories count 0 A 2 1 B 2 2 C 1 3 D 1
...
A subset of the DataFrame representing the first group.

**`g2`** : `pandas.DataFrame`

A subset of the DataFrame representing the second group.

**`col`** : `str`

The name of the column to compare values for.

**`labels`** : `list`

A list of unique values from the column to use as labels.

**`config`** : `dict`

A configuration dictionary with keys: - title (str): The title of the chart. - c1\_label (str): The label for the first group's bars. - c2\_label (str): The label for the second group's bars. - xlabel (str): The label for the x-axis. - ylabel (str): The label for the y-axis.
...
A new DataFrame with two columns: - categories: Contains the unique values from the specified column. - total count: Contains the percentage of occurrences for each unique value.
...
A Series containing numerical values.
...
A Series with the same index as the input, but containing
...
A dictionary containing replacement mappings, where keys represent original values and values represent their replacements.
...
*   ### [Functions](#header-functions)
    ...
    *   `[explode_pie](#eda_helper.explode_pie "eda_helper.explode_pie")`
    *   `[get_column_uniques](#eda_helper.get_column_uniques "eda_helper.get_column_uniques")`
    *   `[get_normal_uniques_col_count](#eda_helper.get_normal_uniques_col_count "eda_helper.get_normal_uniques_col_count")`
    *   `[get_percentage](#eda_helper.get_percentage "eda_helper.get_percentage")`
    *   `[get_uniques_col_count](#eda_helper.get_uniques_col_count "eda_helper.get_uniques_col_count")`
    *   `[make_custom_horizontal_bar](#eda_helper.make_custom_horizontal_bar "eda_helper.make_custom_horizontal_bar")`
    *   `[make_dataframe](#eda_helper.make_dataframe "eda_helper.make_dataframe")`
    *   `[make_df](#eda_helper.make_df "eda_helper.make_df")`
    *   `[make_horizontal_bar](#eda_helper.make_horizontal_bar "eda_helper.make_horizontal_bar")`
    *   `[make_horizontal_grouped_chart](#eda_helper.make_horizontal_grouped_chart "eda_helper.make_horizontal_grouped_chart")`
    *   `[make_normalized_df](#eda_helper.make_normalized_df "eda_helper.make_normalized_df")`
    *   `[make_vertical_grouped_chart](#eda_helper.make_vertical_grouped_chart "eda_helper.make_vertical_grouped_chart")`
    *   `[percentage_to_normal](#eda_helper.percentage_to_normal "eda_helper.percentage_to_normal")`
    *   `[print_column_uniques](#eda_helper.print_column_uniques "eda_helper.print_column_uniques")`
    *   `[random_hex](#eda_helper.random_hex "eda_helper.random_hex")`
    *   `[replace_column_content](#eda_helper.replace_column_content "eda_helper.replace_column_content")`
